...
def create_florida_map(data_path: str, output_path: str, iteration: int) -> None:
    ...
    # Manually adjust position for "WILMA (2005)" - further west and slightly north
    for text_obj in all_texts:
        if text_obj.get_text() == "WILMA (2005)":
            text_obj.set_position((-90, 28.5))
            text_obj.set_ha('right')
            text_obj.set_va('center')
            break

    # Manually adjust position for "GEORGES (1998)" to avoid overlap with "ELENA (1985)" and "OPAL (1995)"
    for text_obj in all_texts:
        if text_obj.get_text() == "GEORGES (1998)":
            text_obj.set_position((-85, 30))
            text_obj.set_ha('left')
            text_obj.set_va('center')
            break

    # Manually adjust position for "DONNA (1960)" to avoid coastline and overlap with "DORIAN (2019)"
    for text_obj in all_texts:
        if text_obj.get_text() == "DONNA (1960)":
            text_obj.set_position((-74, 38))
            text_obj.set_ha('left')
            text_obj.set_va('bottom')
            break

    # Manually adjust position for "DORIAN (2019)" to avoid overlap with "DONNA (1960)"
    for text_obj in all_texts:
        if text_obj.get_text() == "DORIAN (2019)":
            text_obj.set_position((-77, 33))
            text_obj.set_ha('left')
            text_obj.set_va('center')
            break

    # Manually adjust position for "DONNA (1960)" - move further inland and up to avoid coastline overlap
    for text_obj in all_texts:
        if text_obj.get_text() == "DONNA (1960)":
            text_obj.set_position((-75, 39))
            text_obj.set_ha('left')
            text_obj.set_va('bottom')
            break
